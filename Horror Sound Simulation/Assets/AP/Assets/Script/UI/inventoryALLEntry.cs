...
public class inventoryALLEntry : MonoBehaviour {
	...
	private bool b_AccessInventoryInPuzzleDesktop = false;
	private bool b_AccessInventoryInPuzzleGamepad = false;
	...
	public void manageDesktopInputs(){
		...
		//-> Show Diary
                ...
                if(gManager.navigationList.Count > 0 && gManager.b_Joystick && gManager.navigationList[gManager.navigationList.Count - 1] == "Focus" && b_AccessInventoryInPuzzleGamepad
					||
					gManager.navigationList.Count == 0 && gManager.b_Joystick
					||
					gManager.navigationList.Count > 0 && gManager.b_Joystick && gManager.navigationList[gManager.navigationList.Count - 1] != "Focus"
					||
                    !gManager.b_Joystick && b_AccessInventoryInPuzzleDesktop
					||
					!gManager.b_Joystick && gManager.navigationList.Count == 0
                    ||
					gManager.navigationList.Count > 0 && !gManager.b_Joystick && gManager.navigationList[gManager.navigationList.Count - 1] != "Focus"
					)
                {
